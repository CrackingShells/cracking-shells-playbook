...
Explicit non-goal: do **not** change the example spec's `claude_marketplace` or `codex.marketplace_name` declarations. colgrep-mcp does not become hub-mode until the `nest_migration` campaign lands, and until then the example must keep matching the repository it claims to regenerate. Reconciling the example spec to hub mode afterwards is a known seam owned by that campaign, which has been briefed to report it rather than reach across repositories.
