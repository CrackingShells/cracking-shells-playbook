...
**Implementation Logic**:
Find references with `colgrep -e 'committing-changes'` (plain grep/rg are hook-blocked). For each hit, decide: **update** live references (skill indexes like `README.md`, any skill that composes committing-changes, active docs) to `writing-history`; **leave** archival/historical references (`archive/`, `__roadmap__/`, `__reports__/`, `CHANGELOG.md`, `.skill` assets) as-is. Note in the Progress table which files were repointed.
...
**Consistency Checks**: `python3 -c "import os,glob; allow=('archive/','__roadmap__/','__reports__/','dist/','node_modules/','.git/'); bad=[p for p in glob.glob('**/*',recursive=True) if os.path.isfile(p) and not p.startswith(allow) and os.path.basename(p)!='CHANGELOG.md' and 'committing-changes' in open(p,'rb').read().decode('utf-8','ignore')]; assert not bad, bad; print('PASS')"` (expected: PASS)
