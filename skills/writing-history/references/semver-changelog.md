...
A worked example, using this monorepo's own machinery as the concrete case: `release-config.js` at the repo root wires `@semantic-release/commit-analyzer` into each skill's `.releaserc.js` (e.g. `skills/writing-history/.releaserc.js`) without overriding its analyzer preset. That means the *default* preset the tool ships with is this project's authorized mapping — the project chose to inherit a convention rather than define a bespoke one, and that choice is itself the ground truth for this repo. Nothing here should be read as "commit-analyzer's default is universally correct" — it is correct *for this repo* because this repo's own config says so. A different repo's config could remap `perf` to `minor`, drop `revert` bumps entirely, or invent a `data`-type class scoped to dataset commits — all equally valid, because the mapping lives in that project's machinery, not in a skill's assumptions.
...
None of these is *the* answer; each is interchangeable machinery implementing the same principle. As a concrete example already present in this repo, `release-config.js` (repo root) is a shared semantic-release config factory: every skill's `.releaserc.js` calls it with the skill's name, wiring commit-analyzer through changelog generation through a GitHub release asset upload — one worked instance of "structured commits in, changelog and version out," not a template to copy verbatim into unrelated projects.
